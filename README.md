...
`gas/` ディレクトリには、スプレッドシートにメニューを追加するスクリプトが含まれています。編集内容の反映手順を案内するだけで、**デプロイの起動は行いません。**

### なぜスクリプトからデプロイしないのか

Cloud Build API を呼ぶには `cloud-platform` スコープ（Google Cloud 全体の操作）が必要です。スプレッドシートに紐づいたスクリプトは、そのシートを編集できる人なら誰でも書き換えられます。そのため窃取コードを仕込まれると、メニューを押した管理者のトークンがそのまま外部に渡り、Secret Manager の読み出しやサービスアカウント鍵の作成まで可能になります。

Cloud Build の Webhook トリガーを使う方式も採っていません。サービス名はランダムな UUID から作られ、スプレッドシート ID からは導出できないため、呼び出し側がサービス名を指定する必要があります。認証シークレットはシートの編集者が読めるので、他テナントのサービス名を指定して自分のスプレッドシートで再デプロイさせることができてしまいます。

反映は **Makasete AI の管理画面**（別リポジトリ `makasete-ai-platform`）から行います。platform が自身のサービスアカウントでビルドを起動するため、シートの編集者に権限を渡す必要がありません。

> **`gas/main.js` に `ScriptApp.getOAuthToken()` や `UrlFetchApp` を持ち込まないでください。**

### セットアップ

1. 対象のスプレッドシートのメニューから **[拡張機能] > [Apps Script]** を開きます。
2. `gas/main.js` と `gas/appsscript.json` の内容をエディタにコピー＆ペーストします（`appsscript.json` は「プロジェクトの設定」でマニフェストの表示を有効にすると編集できます）。
3. 任意で **[プロジェクトの設定] > [スクリプト プロパティ]** に `DASHBOARD_URL` を登録します。案内ダイアログに管理画面のリンクが表示されます。
4. 保存してスプレッドシートをリロードすると、メニューに **[🤖 Makasete AI]** が追加されます。

必要な権限は現在のスプレッドシートへのアクセスのみです。外部通信も Google Cloud の権限も要求しません。

### 使い方

1. スプレッドシートでプロンプトや商品リストを編集します。
2. Makasete AI の管理画面を開き、対象サーバーの **再ビルド** を実行します。
3. 数分後に最新のスプレッドシート情報が反映されます。

メニューの **[🚀 編集内容の反映方法]** は、この手順を案内するダイアログを表示します。
