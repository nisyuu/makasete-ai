...
- **AI処理**:
    - **LLM (大規模言語モデル)**: Google Gemini API (Gemini Pro) を使用し、ユーザーの意図理解と自然な応答生成を行います。
...
3. **推論 & 検索**: Server -> Gemini (商品情報が必要な場合はGoogle Sheetsを検索)。
4. **応答生成**: Geminiが応答テキストを生成。
5. **音声合成**: 生成されたテキスト -> ElevenLabs -> 音声データ(MP3)。
