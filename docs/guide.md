...
- **`prompt` シート**: AI の性格や接客ルールを記載（必須）。
- **自由な名称のシート**: 商品、FAQ、お知らせなど、任意の名称でシートを追加可能。

| シート名   | 役割                   | 備考                                   |
| :--------- | :--------------------- | :------------------------------------- |
| `prompt`   | システムプロンプト     | A1 セルに指示を記載                    |
| `(任意)`   | 知識ベース             | シート名がそのままカテゴリ名になる     |

※ **任意シート（知識ベース）内**のカラム名や数も自由に変更でき、AI が自動で解釈します。
- 1 シートあたり先頭 **100 行**まで読み込まれます。
- カラム（列）は **A から Z まで**が対象です。
- 1 行目は必ず**ヘッダー（項目名）**として扱われます。
