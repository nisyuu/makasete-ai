...
export class GeminiTTSService implements TTSService {
    ...
    private static readonly VOICE_MAP: Record<string, { languageCode: string; name: string }> = {
    ...
    public async generateSpeechStream(text: string, language = 'ja'): Promise<Readable> {
        ...
        const voice = GeminiTTSService.VOICE_MAP[language] ?? GeminiTTSService.VOICE_MAP['ja'];
